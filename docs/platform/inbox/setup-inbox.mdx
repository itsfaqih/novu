...
description: "Integrate the Novu Inbox component into your application to display real-time in-app notifications for your subscribers with drop-in React or JS components."
---
...
Install `@novu/js`. Mount `NovuUI` from `@novu/js/ui` into a stable element in your header, navbar, or sidebar.
...
- Detect the project's package manager and use it for installation
- Extract design tokens and apply them through the `appearance` option passed to `NovuUI`
- Mount Inbox inline in existing UI - no new pages or framework packages
...
- Add empty appearance values or placeholder props
- Create a published framework wrapper or a new page just for the inbox
- Add code comments or unused imports

## Verify Before Responding

1. Is `@novu/js` installed with the project's package manager?
2. Is `NovuUI` mounted inline in existing UI?
3. Are the application identifier and `subscriber` values non-empty and properly typed?
4. Are the UI renderer and socket cleaned up when the component is destroyed?
